...
export default function Mix({ user }: any): JSX.Element {
  ...
  //uso de navegación entre componenetes
  ...
  const movimiento1 = (event: any) => {
    const { translationX, translationY, x, y, velocityX, velocityY } =
      event.nativeEvent;
        ...
        >
          <View style={styles.modalFondo}>
            <View style={styles.modal}>
              ...
              {nivel == "" ? (
                <Text style={styles.titulo}>{i18n.t("playM")}</Text>
              ) : (
                ""
              )}
              {nivel == "" ? (
              ...
              {nivel == "" ? (
                <Text style={styles.titulo}>{i18n.t("oM")}</Text>
              ) : (
                ""
              )}
              {nivel == "" ? (
...
function revolver(bolas: number) {
  ...
  for (let k = 0; k < arrayBolas.length; k++) {
    ...
    [arrayBolas[k], arrayBolas[ramdomIndex]] = [
      arrayBolas[ramdomIndex],
      arrayBolas[k],
    ];
  }
